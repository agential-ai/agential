...
from discussion_agents.cog.modules.reflecting.generative_agents import (
...
def test_generative_agent_scorer(memory_retriever: BaseRetriever) -> None:
    ...
    reflector = GenerativeAgentReflector(llm=llm, retriever=memory_retriever, importance_weight=0.15)
    insights = reflector.reflect(
        observations=observations, now=test_date
    )
