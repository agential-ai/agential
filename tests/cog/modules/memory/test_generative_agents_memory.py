...
def test_add_memories(memory_retriever: TimeWeightedVectorStoreRetriever) -> None:
    mem = GenerativeAgentMemory(retriever=memory_retriever)
    mem.add_memories(memory_contents="An observation.", importance_scores=0.1, now=test_date)
    ...
    assert str(excinfo.value) == "The length of memory_contents must match the length of importance_scores."
...
def test_load_memories(memory_retriever: TimeWeightedVectorStoreRetriever) -> None:
    ...
    assert str(excinfo.value) == "max_tokens_limit and llm must be defined if consumed_tokens is defined."
    ...
    out = mem.load_memories(consumed_tokens=1, max_tokens_limit=5, llm=FakeListLLM(responses=[""]))
