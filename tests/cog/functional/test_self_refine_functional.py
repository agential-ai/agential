...
def test__prompt_agent() -> None:
    """Test _prompt_agent."""
    out = _prompt_agent(
        llm=FakeListChatModel(responses=["1"]), question="", examples="", prompt=SELF_REFINE_INSTRUCTION_GSM8K
    )
    assert out == "1"
...
def test__prompt_critique() -> None:
    ...
    out = _prompt_critique(
        llm=FakeListChatModel(responses=["1"]), question="", examples="", answer="", prompt=SELF_REFINE_CRITIQUE_INSTRUCTION_GSM8K
    )
    assert out == "1"
...
def test__build_refine_prompt() -> None:
    ...
    out = _build_refine_prompt(question="", examples="", answer="", critique="", prompt=SELF_REFINE_REFINE_INSTRUCTION_GSM8K)
    assert out == gt_out
