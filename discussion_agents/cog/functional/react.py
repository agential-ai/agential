...
def _build_agent_prompt(question: str, scratchpad: str) -> PromptTemplate:
    ...
    prompt = PromptTemplate.from_template(REACT_INSTRUCTION).format(
        examples=WEBTHINK_SIMPLE6, question=question, scratchpad=scratchpad
    )
    return prompt
...
def react_observe(
    ...
    if action_type.lower() == "finish":
        ...
        try:
            ...
            out[
                "scratchpad"
            ] += f"The last page Searched was not found, so you cannot Lookup a keyword in it. Please try one of the similar pages given."
    else:
        out[
            "scratchpad"
