...
def gather_experience(
    ...
    **kwargs: Any,
    ...
    Returns:
        ModelResponse: A list of ExpeLOutput objects, each containing the trajectory and reflections for a given question-key pair.
    """
...
def categorize_experiences(experiences: List[ExpeLOutput]) -> Dict[str, List]:
    ...
    Parameters:
        experiences (List[ExpeLOutput]): A list of ExpeLOutput objects, each containing the trajectory and reflections for a given question-key pair.

    Returns:
        ModelResponse : A dictionary with the indices of tasks categorized into 'compare', 'success', and 'fail'.
...
def get_folds(
    ...
    Returns:
        ModelResponse: A dictionary where keys are fold indices and values are the lists of indices representing the training set for that fold.
    """
    random.seed(seed)
...
def _build_compare_prompt(
    ...
    Returns:
        ModelResponse: A fully constructed prompt ready to be presented to the AI. The prompt includes a prefixed system instruction, task details formatted according to human critique template,
...
def _build_all_success_prompt(
    ...
    Returns:
        ModelResponse: A string that combines the system's instruction, the task context with successful trials, and the existing insights into a coherent prompt.
    """
    # System prompt.
    prefix = SYSTEM_TEMPLATE.format(
        ai_name=(
...
def parse_insights(llm_text: str) -> List[Tuple[str, str]]:
    ...
    Returns:
        ModelResponse: A list of tuples where each tuple contains two elements: the operation (ADD, REMOVE, EDIT, AGREE) and the clean, validated insight description.
...
def retrieve_insight_index(
    ...
    Returns:
        ModelResponse: The index of the rule within the list if found; otherwise, -1.
    """
...
def remove_err_operations(
    ...
    Returns:
        ModelResponse : A cleaned list of operations where erroneous or inapplicable operations have been removed or modified to ensure consistency and correctness when applied to the set of existing insights.
    """
...
def get_operations_compare(
    ...
    Returns:
        ModelResponse: A list of tuples representing operations (e.g., "ADD", "EDIT") and their corresponding insights or modifications.
    """
    # Prompt.
