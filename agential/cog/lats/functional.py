...
from agential.cog.lats.prompts import LATS_INSTRUCTION_HOTPOTQA, COT_PROMPT, COT_PROMPT_FEEDBACK,REFLECTION_PROMPT
...
def get_samples(question, trajectory, thought, additional_keys, n_generate_sample, prompt_sample, stop):
    ...
    if len(unique_trajectories) > len(reflection_map) and len(unique_trajectories) < 4:
        ...
        for traj in failed_trajectories:
            ...
            reflect_prompt = _build_reflection_prompt(trajectory=traj, prompt=REFLECTION_PROMPT)
    ...
    if prompt_sample == 'standard':
        ...
        if reflection_mapping:
            ...
            prompt = _build_cot_prompt(question, trajectory, thought, COT_PROMPT, additional_keys)
    else:
...
def generate_new_states(node, prompt_sample, n):
    ...
    additional_keys = {}
    sampled_actions = get_samples(node.question, trajectory, f"Thought {node.depth + 1}: ", additional_keys, n, prompt_sample=prompt_sample, stop="Observation")
    tried_actions = []
