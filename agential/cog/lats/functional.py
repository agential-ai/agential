...
def _build_reflection_prompt(  
    ...
    """
    prompt = PromptTemplate.from_template(prompt).format(
        question=question, trajectory=trajectory, unique_trajectories=unique_trajectories, **additional_keys
    )
    return prompt
...
def _build_value_prompt(   
    ...
    Parameters:
        question (str): The question to be answered by the agent.
        child_prompts (str): The prompts generated by the agent. 
        unique_trajectories (str): The unique trajectories of the agent. 
        reflections (str): The reflection map created by the agent.
        prompt (str): Prompt template string.
        additional_keys (Dict[str, str]): Additional keys to format the prompt. Defaults to {}.
    ...
    """
    prompt = PromptTemplate.from_template(prompt).format(
        question=question, child_prompts=child_prompts, unique_trajectories=unique_trajectories, reflections=reflections, **additional_keys
    )
    return prompt
