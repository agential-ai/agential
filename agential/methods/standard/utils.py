...
def log_llm_io(
    ...
    """
    Log LLM input/output with rich formatting if available, else fallback to print.
    ...
    try:
        ...
        console = Console()
        input_text = escape(str(getattr(response, 'input_text', '')))
        output_text = escape(str(getattr(response, 'output_text', str(response))))
        ...
        print(f"LLM {context}")
        print("INPUT:", getattr(response, 'input_text', ''))
        print("OUTPUT:", getattr(response, 'output_text', str(response))) 