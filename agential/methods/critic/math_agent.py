...
class CriticMath(BaseMethod):
    ...
    Attributes:
        ...
        benchmark (str): The benchmark name (gsm8k, svamp, tabmwp)
        use_execution (bool): Whether to use code execution for validation
        patience (int): Number of interactions to tolerate same incorrect answer
    ...
    def __init__(
        ...
        """Initialize the CRITIC Math Agent.

        Args:
            llm: The language model to use for generation
            benchmark: The benchmark name (gsm8k, svamp, tabmwp)
            use_execution: Whether to use code execution for validation
            patience: Number of interactions to tolerate same incorrect answer
        ...
        for idx in range(max_interactions):
            ...
            new_critique, external_tool_info, finished, critique_responses = (
                self._generate_critique(
                    ...
                    prev_code_answer=prev_code_answer,
                    patience_counter=patience_counter,
                )
            )

            # Update tracking variables
            if use_execution and external_tool_info.get("code_answer") is not None:
                answer_history.append(
                    {"answer": answer, "external_tool_info": external_tool_info}
                )
                code_answer = external_tool_info["code_answer"]

                if code_answer == prev_code_answer:
                    patience_counter += 1
                    if patience_counter == self.patience:
                        finished = True
                else:
                    prev_code_answer = code_answer
                    patience_counter = 0
            ...
            if not finished:
                ...
                # If finished, use the last valid code answer
                if use_execution and answer_history:
                    last_valid_answer = None
                    for hist in reversed(answer_history):
                        if hist["external_tool_info"]["code_answer"] is not None:
                            last_valid_answer = hist["answer"]
                            break
    ...
    def _generate_critique(
        ...
        Args:
            ...
            answer_history: History of previous answers
            prev_code_answer: Previous code execution result
        ...
        if use_execution:
            # Execute the code
            code_answer, execution_status = safe_execute(answer_code)
            external_tool_info = {
            ...
            if code_answer[0] == prev_code_answer:
                patience_counter += 1
                if patience_counter == self.patience:
                    finished = True
            else:
                prev_code_answer = code_answer[0]

            # Find last valid answer from history
            last_valid_idx = -1
            for i in range(len(answer_history) - 1, -1, -1):
                if answer_history[i]["external_tool_info"]["code_answer"] is not None:
                    last_valid_idx = i
                    break

            if last_valid_idx >= 0:
                external_tool_info = answer_history[last_valid_idx][
                    "external_tool_info"
                ]
                answer_code = (
                    answer_history[last_valid_idx]["answer"]
                    .split("```python")[-1]
                    .split("```")[0]
                    .strip()
                )
