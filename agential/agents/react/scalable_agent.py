...
@dataclass
class StepMetrics:
    """Structured metrics for a single step."""
    step_number: int
    thought_tokens: int = 0
    thought_cost: float = 0.0
    thought_time: float = 0.0
    action_tokens: int = 0
    ...
    def __post_init__(self):
        self.total_tokens = self.thought_tokens + self.action_tokens
        self.total_cost = self.thought_cost + self.action_cost
        self.total_time = self.thought_time + self.action_time


@dataclass
class AgentMetrics:
    """Structured metrics for the entire agent run."""
    ...
    def add_step(self, step_metrics: StepMetrics):
        ...
        self.total_tokens += step_metrics.total_tokens
        self.total_cost += step_metrics.total_cost
        self.total_time += step_metrics.total_time
            ...
            "steps": [
                {
                    ...
                    "thought_cost": step.thought_cost,
                    "thought_time": step.thought_time,
                    "action_tokens": step.action_tokens,
...
class AgentLogger:
    ...
    def log_start(self, benchmark: str, question: str):
        ...
        if self.use_rich:
            ...
            print(f"🤖 ReAct Agent Starting - {benchmark}")
            print(f"Question: {question[:100]}{'...' if len(question) > 100 else ''}")
            print(f"Time: {self.metrics.start_time.strftime('%H:%M:%S')}")
            print("-" * 80)
    ...
    def log_step(self, step_number: int, thought: str, action_type: str, query: str, 
        ...
        if self.use_rich:
            ...
            print(f"⚡ Action: {action_type}[{query[:100]}{'...' if len(query) > 100 else ''}]")
            print(f"📊 Tokens: {step_metrics.total_tokens}, Cost: ${step_metrics.total_cost:.4f}, Time: {step_metrics.total_time:.2f}s")
    ...
    def log_observation(self, observation: str, finished: bool = False):
        ...
        if self.use_rich:
            ...
            if finished:
                ...
                print(f"👁️ {observation[:200]}{'...' if len(observation) > 200 else ''}")
    ...
    def log_finish(self, answer: str):
        ...
        if self.use_rich:
            ...
            print(f"Average Time/Step: {self.metrics.total_time/max(1, self.metrics.total_steps):.2f}s")
...
class ScalableReAct(BaseAgent):
    """Scalable ReAct agent that uses plugin-based handlers for benchmarks.
    
    This architecture makes it extremely easy to add new benchmarks:
    
    1. Create a new handler class inheriting from QAHandler, MathHandler, or CodeHandler
    2. Add it to BENCHMARK_HANDLERS registry
    3. Add prompt to prompts.py
    4. Add few-shot examples to constants.py
    
    Example of adding a new benchmark:
    
    ```python
    # 1. Create handler
    class MyNewBenchmarkHandler(QAHandler):
        def get_prompt(self) -> str:
    ...
    # 3. Use it
    agent = ScalableReAct(llm, "my_new_benchmark")
    ```
    ...
    def get_prompts(benchmark: str, **kwargs: Any) -> Dict[str, str]:
        ...
        handler_class = BENCHMARK_HANDLERS[benchmark]
        # Create a temporary handler to get the prompt
        temp_handler = handler_class(llm=BaseLLM(), max_steps=6)  # Use dummy LLM
        return {"prompt": temp_handler.get_prompt()}
    
    @staticmethod
    def register_benchmark(benchmark_name: str, handler_class: Type[BenchmarkHandler]) -> None:
        """Register a new benchmark handler.
        
        This is the key method that makes the architecture scalable!
        
        Args:
            benchmark_name: Name of the benchmark
            handler_class: Handler class for the benchmark
        """
