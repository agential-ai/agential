    ...
    def generate(
        ...
        for trial in range(1, self.max_trials + 1):
            ...
            for idx in range(1, self.max_steps + 1):
                ...
                match = re.match(r"^(\w+)\[(.*)\]$", action_type.strip(), re.DOTALL)
                if match:
                    action_type = match.group(1)
                    query = match.group(2).strip()
                else:
                    ...
                    if match:
                        ...
                        action_type = action_type.strip().split()[0] if action_type.strip() else ""
                        query = action_type.strip()[len(action_type):].strip()
                # Handle observation
                scratchpad += f"\nObservation {idx}: "
